...
function charge()
{
  ...
  document.getElementById("exo_consigne").value = "Coche la bonne réponse.";
...
function change(elem)
{
  if (elem.id == "exo_type")
  {
    if (elem.value == "btn")
    {
      document.getElementById("btn_coul").style.visibility = "visible";
    }
    else
    {
      document.getElementById("btn_coul").style.visibility = "hidden";
    }
  }
...
function sauvegarder()
{
  ...
  // et le fichier html
  htm = "";
  tp = document.getElementById("exo_type").value;
  if (tp == "btn") tp = "radio";
  item = 0;
  id = 0;
  for (var i=0; i<lg.length; i++)
  {
    ...
    while (j<vals.length)
    {
      ...
      else
      {
        ...
        htm += "<div class=\"choix\">\n";
        htm += "<form>\n";
        v = vals[j].replace(/\*/g,"|*").replace(/\$/g,"|$").split("|");
        for (var k=0; k<v.length; k++)
        {
          if (v[k].startsWith("*"))
          {
            if (k>1) htm += "<br/>";
            htm += "<input type=\"" + tp + "\" itemid=\"" + item + "\" class=\"exo\" onclick=\"change(this)\" value=\"1\" id=\"rb" + id + "\" ";
            if (tp == "radio") htm += "name=\"" + item + "\"";
            htm += ">\n";
            htm += "<label for=\"rb" + id + "\">" + v[k].substr(1) + "</label>\n";
            id++;
          }
          else if (v[k].startsWith("$"))
          {
            if (k>1) htm += "<br/>";
            htm += "<input type=\"" + tp + "\" itemid=\"" + item + "\" class=\"exo\" onclick=\"change(this)\" value=\"0\" id=\"rb" + id + "\" ";
            if (tp == "radio") htm += "name=\"" + item + "\"";
            htm += ">\n";
            htm += "<label for=\"rb" + id + "\">" + v[k].substr(1) + "</label>\n";
            id++;
  ...
  if (document.getElementById("exo_type").value == "btn")
  {
    txt = getcss2();
    txt = txt.replace("######", document.getElementById("btn_coul1").value);
    txt = txt.replace("######", document.getElementById("btn_coul2").value);
    zip.file("exo.css", txt);
  }
  else zip.file("exo.css", getcss());
